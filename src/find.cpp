...
//utility class for Kruskal generate
class Set
{
    public:
...
int main(int argc, char* argv[])
{
    ...
    SDL_CreateWindowAndRenderer(n*15, n*15, 0, &window, &renderer);
    ...
    //generate
    int start = 0, end = 0;
    generateRecursive(renderer, grid, n/2, n/2, start, end);
    solveRecursive(renderer, grid, start, end); 
    solveDE(renderer, grid, start, end); 
    solveBFS(renderer, grid, start, end); 
    solveAStar(renderer, grid, start, end); 
    resetAll(renderer, grid);
    generatePrims(renderer, grid, 1, 1, start, end);
    solveRecursive(renderer, grid, start, end); 
    solveDE(renderer, grid, start, end); 
    solveBFS(renderer, grid, start, end); 
    solveAStar(renderer, grid, start, end); 
    resetAll(renderer, grid);
    generateKruskal(renderer, grid, 1, 1, start, end);
    solveRecursive(renderer, grid, start, end); 
    solveDE(renderer, grid, start, end); 
    solveBFS(renderer, grid, start, end); 
    solveAStar(renderer, grid, start, end); 
